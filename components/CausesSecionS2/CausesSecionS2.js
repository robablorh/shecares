                            ...
                            {causesData.slice(0, 1).map((cause, index) => (
                                <div key={index} className="causes-item">
                                    ...
                                    <div className="content">
                                        <h2><Link onClick={ClickHandler} href={'/causes-single/[slug]'} as={`/causes-single/${cause.slug}`}>{cause.title}</Link></h2>
                        ...
                        <div className="col col-lg-6 col-md-12 col-12">
                            {causesData.slice(1, 3).map((cause, index) => (
                                <div key={index} className="causes-item">
                                    ...
                                    <div className="content">
                                        <h2><Link href={'/causes-single/[slug]'} as={`/causes-single/${cause.slug}`}>{cause.title}</Link></h2>
                                        <p>{cause.description}</p>
                                        <div className="progress-item">
                                            <div className="progress">
                                                <div className="bar" style={{ width: `${(cause.raisedAmount / cause.goalAmount) * 100}%` }}></div>
                            ...
                            <div className="causes-item-btn">
                                <Link href="/causes-single" className="causes-btn" onClick={ClickHandler} >See All Causes <i className="flaticon-right-arrow-2"></i></Link>
                            </div>
                        </div>
                    </div>
                </div>
